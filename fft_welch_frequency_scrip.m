...
path = '/home/np451/Desktop/ependymal/June2018/31.5.18/frequency/';
cd(path);
mkdir('analysis_fft');
ana_dir= strcat(path,'analysis_fft/');
store_dir= '/run/user/10704/gvfs/smb-share:server=sf3.bss.phy.private.cam.ac.uk,share=space/np451/31.5.18/';
...
% fft window for welch 
     N_seg=4;
     overlap=50;
     window = hann(floor(Nframes)/N_seg);
    temp=double(squeeze(fs(1,1,:)));
    [~, dummyf]=pwelch(temp, window,overlap,size(temp,1),FR);
    Nfreqs = numel(dummyf);
    clear dummyf;
...
for cc = 1:ceil(width/bsz)
    ...
    % take the first block, reshape it to be a Nframes, height*box matrix
...
%    [pxx, frequencies] = periodogram(temp_fs,window,size(temp_fs,1),FR);
    [pxx, frequencies] = pwelch(double(temp_fs), window,overlap,size(temp_fs,1),FR);
